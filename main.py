...
def processar_pergunta(pergunta: str, sintetizador, servidor_mcp) -> str:
    ...
    print("🔍 [Recuperador] Buscando contexto relevante...")
    ...
    total = len(contextos["docs_python"]) + len(contextos["stackoverflow"])
    print(f"   ✓ {total} trechos recuperados\n")

    # Passo 2: Sintetizar resposta
    print("✍️  [Sintetizador] Gerando resposta...")
    resposta = sintetizador.gerar(pergunta, contextos)
    print("   ✓ Resposta gerada\n")

    # Passo 3: Revisar coerência via MCP
    print("🔎 [Revisor] Verificando coerência...")
    ...
    if avaliacao.get("aprovado", True):
        ...
        print("   ✗ Reprovada — regenerando...\n")
        print("✍️  [Sintetizador] Nova tentativa...")
        resposta = sintetizador.gerar(pergunta, contextos)
        print("   ✓ Resposta regenerada\n")
